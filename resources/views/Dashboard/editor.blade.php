...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@500;600;700&family=Cormorant+Garamond:ital,wght@0,500;0,600;0,700;1,400&family=Great+Vibes&family=Montserrat:wght@400;500;600;700&family=Playfair+Display:ital,wght@0,500;0,600;0,700;1,400&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
    <!-- Phosphor Icons -->
    ...
    <style>
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #E27367; border-radius: 10px; }
        ...
        .canvas-item.is-locked { cursor: default !important; }
        .canvas-item.active-item {
            outline: 2px dashed #E27367 !important;
            outline-offset: 4px;
            z-index: 50 !important;
        }
        .canvas-item.active-item.is-locked { outline: 2px dashed #7A6660 !important; }
        ...
        .spin-vinyl { animation: spinSlow 4s linear infinite; }
    </style>
</head>
...
<svg class="absolute w-0 h-0" aria-hidden="true" focusable="false">
    <defs>
        <clipPath id="heartClipPath" clipPathUnits="objectBoundingBox">
            <path d="M 0.5, 0.95 C 0.5, 0.95 0.03, 0.62 0.01, 0.35 C -0.01, 0.16 0.14, 0.02 0.32, 0.02 C 0.42, 0.02 0.47, 0.08 0.5, 0.14 C 0.53, 0.08 0.58, 0.02 0.68, 0.02 C 0.86, 0.02 1.01, 0.16 0.99, 0.35 C 0.97, 0.62 0.5, 0.95 0.5, 0.95 Z"></path>
        </clipPath>
    </defs>
</svg>
...
<div class="flex min-h-screen">
    ...
    <div class="ml-64 w-full flex flex-col h-screen overflow-hidden">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-rosewarm-100 px-8 flex items-center justify-between z-20 flex-shrink-0">
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="font-playfair font-bold text-xl text-darkcharcoal">Studio Editor Kustomisasi Desain 🎨</h1>
                    <span class="bg-rosewarm-100 text-rosewarm-500 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Free Canvas Editor</span>
                </div>
                <p class="text-xs text-warmgray mt-0.5">{{ $design['groom_short'] ?? 'Dimas' }} & {{ $design['bride_short'] ?? 'Sarah' }} &bull; Custom Audio & Canvas</p>
            </div>

            <!-- ACTION CONTROLS -->
            <div class="flex items-center gap-3">
                <a href="{{ route('daftartamu') ?? '/guests' }}" class="px-4 py-2.5 rounded-full border border-rosewarm-200 bg-white text-darkcharcoal text-xs font-semibold hover:bg-rosewarm-50 transition shadow-sm flex items-center gap-1.5">
                    <i class="ph-bold ph-users"></i> Ke Daftar Tamu
                </a>
                <button type="button" onclick="clearCanvas()" class="px-4 py-2.5 rounded-full border border-rosewarm-200 bg-white text-darkcharcoal text-xs font-semibold hover:bg-rosewarm-50 transition shadow-sm flex items-center gap-1.5">
                    <i class="ph-bold ph-trash"></i> Bersihkan Kanvas
                </button>
                <button type="button" onclick="prepareSubmit()" class="px-5 py-2.5 rounded-full bg-gradient-to-r from-terracotta to-rosewarm-500 hover:opacity-95 text-white text-xs font-semibold flex items-center gap-2 shadow-lg shadow-rosewarm-200 transition">
        ...
        <div class="bg-{{ (session('status_type') ?? $status_type) === 'success' ? 'emerald-600' : 'rosewarm-500' }} text-white text-xs font-semibold px-8 py-2.5 flex items-center justify-between flex-shrink-0 transition-all">
            <div class="flex items-center gap-2">
                <i class="ph-bold {{ (session('status_type') ?? $status_type) === 'success' ? 'ph-check-circle' : 'ph-warning-circle' }} text-base"></i>
                <span>{{ session('status_msg') ?? $status_msg }}</span>
            </div>
            <button onclick="this.parentElement.remove()" class="text-white/80 hover:text-white"><i class="ph-bold ph-x"></i></button>
        </div>
        @endif

        <!-- MAIN WORKSPACE FORM -->
        <form id="editorForm" action="{{ route('editor.save') }}" method="POST" enctype="multipart/form-data" class="flex-1 flex overflow-hidden">
            @csrf

            <input type="hidden" name="theme" id="selectedTheme" value="{{ $design['theme'] ?? 'warm-terracotta' }}">
            ...
            <input type="hidden" name="bg_music_title" id="bgMusicTitle" value="{{ $design['bg_music_title'] ?? '' }}">
            <input type="hidden" name="bg_music_url" id="bgMusicUrl" value="{{ $design['bg_music_url'] ?? '' }}">

            <!-- LEFT CONTROL SIDEBAR -->
            <aside class="w-[500px] bg-white border-r border-rosewarm-100 flex flex-col z-10 flex-shrink-0 shadow-sm">
                
                <!-- TAB HEADERS -->
                <div class="flex border-b border-rosewarm-100 bg-sand/40 p-2 gap-1 flex-shrink-0 overflow-x-auto custom-scrollbar">
                    <button type="button" onclick="switchEditorTab('add-elements')" id="tabBtn-add-elements" class="tab-btn flex-1 py-2 px-2.5 rounded-xl text-xs font-semibold text-rosewarm-500 bg-white shadow-sm flex items-center justify-center gap-1 transition whitespace-nowrap">
                        <i class="ph-bold ph-plus-circle text-sm"></i> Elemen
                    </button>
                    <button type="button" onclick="switchEditorTab('media')" id="tabBtn-media" class="tab-btn flex-1 py-2 px-2.5 rounded-xl text-xs font-semibold text-warmgray hover:text-darkcharcoal flex items-center justify-center gap-1 transition whitespace-nowrap">
                        <i class="ph-bold ph-images text-sm"></i> Foto
                    </button>
                    <button type="button" onclick="switchEditorTab('music')" id="tabBtn-music" class="tab-btn flex-1 py-2 px-2.5 rounded-xl text-xs font-semibold text-warmgray hover:text-darkcharcoal flex items-center justify-center gap-1 transition whitespace-nowrap">
                        <i class="ph-bold ph-music-notes text-sm"></i> Musik
                    </button>
                    <button type="button" onclick="switchEditorTab('active-layers')" id="tabBtn-active-layers" class="tab-btn flex-1 py-2 px-2.5 rounded-xl text-xs font-semibold text-warmgray hover:text-darkcharcoal flex items-center justify-center gap-1 transition whitespace-nowrap">
                        <i class="ph-bold ph-stack text-sm"></i> Layer (<span id="layerCount">0</span>)
                    </button>
                    <button type="button" onclick="switchEditorTab('styling')" id="tabBtn-styling" class="tab-btn flex-1 py-2 px-2.5 rounded-xl text-xs font-semibold text-warmgray hover:text-darkcharcoal flex items-center justify-center gap-1 transition whitespace-nowrap">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div id="panel-add-elements" class="editor-panel space-y-5">
                        <div>
                            <h3 class="font-playfair text-base font-bold text-darkcharcoal">Pilih & Masukkan Elemen</h3>
                            <p class="text-xs text-warmgray mt-0.5">Klik salah satu tombol di bawah untuk memasukkan elemen ke kanvas.</p>
                        </div>

                        <div class="space-y-2.5">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Elemen Teks Undangan</label>
                            
                            <div class="grid grid-cols-2 gap-2.5">
                                <button type="button" onclick="addElement('badge', 'THE WEDDING OF')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('groom_name', 'Dimas Anggara')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('ampersand', '&')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('bride_name', 'Sarah Amalia')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('date', 'Sabtu, 18 Desember 2026')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('venue', 'The Plataran Garden, Jakarta')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                    <i class="ph-bold ph-map-pin text-terracotta text-lg"></i>
                                    <div>
                                        <span class="text-xs font-bold text-darkcharcoal block">Lokasi & Gedung</span>
                                        <span class="text-[9px] text-warmgray">Nama Venue</span>
                                    </div>
                                </button>

                                <button type="button" onclick="addElement('quote', '“Dan di antara tanda-tanda kebesaran-Nya ialah Dia menciptakan pasangan untukmu...”')" class="col-span-2 p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                    <i class="ph-bold ph-quotes text-terracotta text-lg"></i>
                                    <div>
                                        <span class="text-xs font-bold text-darkcharcoal block">Kutipan Romantis / Ayat</span>
                        ...
                        <div class="space-y-2.5 pt-3 border-t border-rosewarm-100">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Komponen & Kartu Interaktif</label>
                            
                            <div class="grid grid-cols-2 gap-2.5">
                                <button type="button" onclick="addElement('guest_card', 'Nama Tamu Undangan')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('rsvp_button', 'Konfirmasi Kehadiran (RSVP)')" class="p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                ...
                                <button type="button" onclick="addElement('gift_card', 'BCA - 8410293810 (a.n Dimas)')" class="col-span-2 p-3 bg-sand/30 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-100 text-left transition flex items-center gap-2.5">
                                    <i class="ph-bold ph-gift text-terracotta text-lg"></i>
                                    <div>
                                        <span class="text-xs font-bold text-darkcharcoal block">Kotak Amplop Digital</span>
                        ...
                        <div class="pt-3 border-t border-rosewarm-100 space-y-2">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Tambah Teks Kustom Bebas</label>
                            <div class="flex gap-2">
                                <input type="text" id="customTextInput" placeholder="Ketik teks kustom..." class="flex-1 px-3.5 py-2 rounded-xl bg-sand/40 border border-rosewarm-200 text-xs">
                                <button type="button" onclick="addCustomText()" class="px-4 py-2 bg-terracotta text-white rounded-xl text-xs font-semibold hover:bg-rosewarm-600 transition">
                                    + Tambah
                                </button>
                            </div>
                        </div>
                    ...
                    <div id="panel-media" class="editor-panel hidden space-y-5">
                        <div>
                            <h3 class="font-playfair text-base font-bold text-darkcharcoal">Galeri Aset & Tambah Foto</h3>
                            <p class="text-xs text-warmgray mt-0.5">Unggah foto lalu klik bentuk bingkai di bawah untuk memasukkannya ke kanvas.</p>
                        </div>

                        <div id="dropZone" ondragover="handleDragOver(event)" ondragleave="handleDragLeave(event)" ondrop="handleDrop(event)" class="border-2 border-dashed border-rosewarm-200 rounded-2xl p-5 text-center bg-sand/30 hover:bg-rosewarm-50/50 transition relative cursor-pointer">
                            <input type="file" id="multiFileInput" multiple accept="image/*" onchange="handleMultipleFiles(event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            <div class="space-y-1.5 pointer-events-none">
                                ...
                                <p class="text-[10px] text-warmgray">atau <span class="text-terracotta underline font-semibold">klik untuk upload</span></p>
                            </div>
                        </div>

                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Pustaka Aset Foto (<span id="photoCount">0</span>)</label>
                            <div id="galleryGrid" class="grid grid-cols-4 gap-2 max-h-44 overflow-y-auto custom-scrollbar p-1"></div>
                        </div>

                        <div class="pt-3 border-t border-rosewarm-100 space-y-2">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Pilihan Bentuk Bingkai Foto</label>
                            <div class="grid grid-cols-4 gap-2">
                                <button type="button" onclick="addPhotoFrame('shape-heart')" class="p-2 bg-rosewarm-100 hover:bg-rosewarm-200 rounded-xl border border-rosewarm-300 text-center text-xs font-bold text-terracotta shadow-sm">💖 Love</button>
                                <button type="button" onclick="addPhotoFrame('shape-rect')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">▬ Persegi</button>
                                <button type="button" onclick="addPhotoFrame('shape-arch')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">🏛️ Arch</button>
                                <button type="button" onclick="addPhotoFrame('shape-circle')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">⚪ Circle</button>
                                <button type="button" onclick="addPhotoFrame('shape-rounded')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">🔲 Card</button>
                                <button type="button" onclick="addPhotoFrame('shape-oval')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">🥚 Oval</button>
                                <button type="button" onclick="addPhotoFrame('shape-wave')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">🌊 Wave</button>
                                <button type="button" onclick="addPhotoFrame('shape-polaroid')" class="p-2 bg-sand/40 hover:bg-rosewarm-50 rounded-xl border border-rosewarm-200 text-center text-xs font-medium">📷 Polaroid</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="panel-music" class="editor-panel hidden space-y-5">
                        <div>
                            <h3 class="font-playfair text-base font-bold text-darkcharcoal">Musik Latar Undangan (Audio File)</h3>
                            <p class="text-xs text-warmgray mt-0.5">Unggah file lagu audio MP3/WAV dari perangkat Anda.</p>
                        </div>

                        <div class="border-2 border-dashed border-rosewarm-200 rounded-2xl p-6 text-center bg-sand/30 hover:bg-rosewarm-50/50 transition relative cursor-pointer">
                            <input type="file" id="customAudioInput" accept="audio/*" onchange="handleCustomAudioUpload(event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            <div class="space-y-2 pointer-events-none">
                                <div class="w-12 h-12 rounded-full bg-rosewarm-100 text-terracotta flex items-center justify-center mx-auto">
                                    <i class="ph-bold ph-music-notes text-2xl"></i>
                                </div>
                                <div>
                                    <p class="text-xs font-bold text-darkcharcoal">Pilih File Audio dari Perangkat</p>
                                    <p class="text-[10px] text-warmgray mt-0.5">Mendukung format MP3, WAV, AAC, M4A</p>
                                </div>
                            </div>
                        </div>

                        <div id="audioActiveContainer" class="p-4 rounded-2xl bg-white border border-rosewarm-200 shadow-sm space-y-3.5 {{ empty($design['bg_music_url']) ? 'hidden' : '' }}">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div id="audioDiscIcon" class="w-11 h-11 rounded-full bg-darkcharcoal text-white flex items-center justify-center shadow-md">
                                        <i class="ph-fill ph-disc text-xl text-rosewarm-300"></i>
                                    </div>
                                    <div class="max-w-[200px]">
                                        <span id="currentMusicLabel" class="text-xs font-bold text-darkcharcoal block truncate">{{ $design['bg_music_title'] ?? 'Lagu Terpilih' }}</span>
                                        <span id="musicStatusText" class="text-[10px] text-warmgray">Status: Siap diputar</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2">
                                    <button type="button" id="previewPlayBtn" onclick="toggleAudioPreview()" class="w-9 h-9 rounded-full bg-terracotta text-white flex items-center justify-center hover:bg-rosewarm-600 transition shadow">
                                        <i id="playIcon" class="ph-fill ph-play text-base"></i>
                                    </button>
                                    <button type="button" onclick="removeAudio()" class="w-8 h-8 rounded-full bg-stone-100 text-stone-400 hover:text-red-500 flex items-center justify-center transition" title="Hapus Lagu">
                        ...
                        <div id="audioEmptyNotice" class="p-4 text-center text-xs text-warmgray border border-dashed border-rosewarm-200 rounded-xl bg-sand/20 {{ !empty($design['bg_music_url']) ? 'hidden' : '' }}">
                            Belum ada file musik yang dipilih.
                        </div>

                        <div class="pt-3 border-t border-rosewarm-100 space-y-2">
                            <label class="flex items-center justify-between p-3 rounded-xl bg-white border border-stone-200 cursor-pointer">
                                <div>
                                    <p class="text-xs font-semibold text-darkcharcoal">Auto-Play Lagu</p>
                                    <p class="text-[10px] text-warmgray">Putar otomatis saat tamu membuka undangan</p>
                                </div>
                                <input type="checkbox" id="autoPlayMusicCheck" checked class="w-4 h-4 accent-terracotta rounded">
                            </label>
                        </div>
                    </div>
                    ...
                    <div id="panel-active-layers" class="editor-panel hidden space-y-4">
                        <div>
                            <h3 class="font-playfair text-base font-bold text-darkcharcoal">Daftar Layer & Kunci Posisi</h3>
                            <p class="text-xs text-warmgray mt-0.5">Ubah teks, ukuran, bentuk, posisi & zoom foto di dalam bingkai.</p>
                        </div>
                    ...
                    <div id="panel-styling" class="editor-panel hidden space-y-5">
                        <div>
                            <h3 class="font-playfair text-base font-bold text-darkcharcoal">Pengaturan Kanvas & Gaya Visual</h3>
                            <p class="text-xs text-warmgray mt-0.5">Sesuaikan panjang scroll halaman, tema warna background, dan font.</p>
                        </div>

                        <div class="bg-rosewarm-50/50 p-4 rounded-2xl border border-rosewarm-100 space-y-3">
                            <div class="flex items-center justify-between">
                                <div>
                                    <label class="text-xs font-bold text-darkcharcoal block">Panjang / Tinggi Kanvas (Scroll)</label>
                                    <span class="text-[10px] text-warmgray">Atur tinggi total konten undangan</span>
                                </div>
                                <div class="flex items-center gap-1">
                                    <input type="number" id="canvasHeightInput" min="600" max="3500" step="50" value="1200" oninput="updateCanvasHeight(this.value)" class="w-16 px-2 py-1 bg-white border border-rosewarm-200 rounded-lg text-xs font-bold text-center">
                                    <span class="text-[10px] text-warmgray font-semibold">px</span>
                                </div>
                            </div>
                            <input type="range" id="canvasHeightSlider" min="600" max="3000" step="50" value="1200" oninput="updateCanvasHeight(this.value)" class="w-full accent-terracotta cursor-pointer">
                        </div>

                        <div class="space-y-3 pt-2 border-t border-rosewarm-100">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Tipe Warna Latar (Background)</label>
                            
                            <div class="grid grid-cols-2 gap-2 bg-sand/50 p-1 rounded-xl">
                                <button type="button" onclick="setBackgroundMode('solid')" id="btnBgModeSolid" class="py-1.5 rounded-lg text-xs font-bold bg-white text-darkcharcoal shadow-sm transition">Warna Solid</button>
                                <button type="button" onclick="setBackgroundMode('gradient')" id="btnBgModeGradient" class="py-1.5 rounded-lg text-xs font-semibold text-warmgray hover:text-darkcharcoal transition">Gradasi (Gradient)</button>
                            </div>

                            <div id="solidBgControls" class="p-3 bg-sand/30 rounded-xl border border-rosewarm-100 flex items-center justify-between">
                                <div>
                                ...
                                <input type="color" id="solidBgColorPicker" value="#FDFBF7" oninput="applySolidBg(this.value)" class="w-9 h-9 rounded-lg cursor-pointer bg-transparent border-0">
                            </div>

                            <div id="gradientBgControls" class="hidden space-y-2.5 p-3 bg-sand/30 rounded-xl border border-rosewarm-100">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="p-2 bg-white rounded-lg border border-rosewarm-100 flex items-center justify-between">
                                        <span class="text-[10px] font-semibold">Warna 1</span>
                                        <input type="color" id="gradColor1" value="#FFF5F5" oninput="applyGradientBg()" class="w-7 h-7 rounded cursor-pointer bg-transparent border-0">
                                    </div>
                                    <div class="p-2 bg-white rounded-lg border border-rosewarm-100 flex items-center justify-between">
                                        <span class="text-[10px] font-semibold">Warna 2</span>
                                        <input type="color" id="gradColor2" value="#FDE8E8" oninput="applyGradientBg()" class="w-7 h-7 rounded cursor-pointer bg-transparent border-0">
                                    </div>
                                </div>
                                <div>
                                    <span class="text-[10px] font-semibold text-warmgray block mb-1">Arah Gradasi</span>
                                    <select id="gradDirection" onchange="applyGradientBg()" class="w-full px-3 py-1.5 rounded-lg bg-white border border-rosewarm-100 text-xs font-medium">
                        ...
                        <div class="space-y-3 pt-2 border-t border-rosewarm-100">
                            <label class="text-[11px] font-bold text-darkcharcoal uppercase tracking-wider block">Gaya Tipografi & Warna Font Global</label>

                            <div>
                                <label class="text-[11px] font-semibold text-warmgray block mb-1">Pilihan Font Family Default</label>
                                <select id="globalFontFamily" onchange="updateGlobalFont(this.value)" class="w-full px-3.5 py-2.5 rounded-xl bg-sand/50 border border-rosewarm-200 text-xs font-medium text-darkcharcoal">
                                    <option value="font-playfair">Playfair Display (Klasik Romantis Serif)</option>
                                    <option value="font-cormorant">Cormorant Garamond (Vintage Luxury Serif)</option>
                            ...
                            <div class="p-3 bg-sand/30 rounded-xl border border-rosewarm-100 flex items-center justify-between">
                                <div>
                                    <span class="text-xs font-bold text-darkcharcoal block">Ubah Semua Warna Teks</span>
                                    <span class="text-[10px] text-warmgray">Terapkan warna ini ke seluruh elemen teks</span>
                                </div>
                                <input type="color" id="globalTextColorPicker" value="#2D2422" oninput="applyGlobalFontColor(this.value)" class="w-9 h-9 rounded-lg cursor-pointer bg-transparent border-0">
                            </div>
                        </div>
                    </div>
            ...
            <main class="flex-1 bg-stone-100 flex flex-col items-center justify-center p-6 overflow-y-auto relative">
                
                <!-- TOP TOOLBAR -->
                <div class="mb-2.5 bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full border border-rosewarm-200 shadow-sm flex items-center gap-4 text-xs z-30">
                    <label class="flex items-center gap-1.5 cursor-pointer select-none text-[11px] font-semibold text-darkcharcoal">
                        <input type="checkbox" id="toggleSnap" checked onchange="toggleSnapMode(this.checked)" class="w-3.5 h-3.5 accent-terracotta rounded cursor-pointer">
                        <i class="ph-bold ph-magnet text-terracotta text-sm"></i>
                        <span>Snap Garis Tengah</span>
                    </label>
                    <span class="text-stone-300">|</span>
                    <button type="button" onclick="centerActiveElement()" class="text-warmgray hover:text-terracotta text-[11px] font-medium flex items-center gap-1 transition">
                        <i class="ph-bold ph-align-center-horizontal text-sm"></i> Ratakan Tengah Elemen Aktif
                    </button>
                </div>

                <!-- Phone Frame -->
                <div id="previewWrapper" class="w-[340px] h-[640px] bg-darkcharcoal rounded-[44px] p-3 shadow-2xl border-4 border-stone-800 relative flex flex-col">
                    
                    <div class="w-32 h-4 bg-stone-900 rounded-b-2xl mx-auto z-30 flex-shrink-0"></div>

                    <div id="phoneScreen" onclick="deselectAll(event)" class="w-full flex-1 rounded-[34px] overflow-y-auto custom-scrollbar relative font-playfair transition-all duration-300 min-h-[600px]" style="background-color: #FDFBF7;">
                        
                        <div id="centerGuideLine"></div>

                        <button type="button" id="phoneMusicDisc" onclick="toggleAudioPreview()" class="absolute bottom-4 right-4 z-40 w-9 h-9 rounded-full bg-darkcharcoal/90 text-white flex items-center justify-center shadow-lg border border-white/20 transition hover:scale-105 {{ empty($design['bg_music_url']) ? 'hidden' : '' }}" title="Musik Latar">
                            <i id="phoneDiscIcon" class="ph-fill ph-disc text-lg text-rosewarm-300"></i>
                        </button>

                        <div id="emptyCanvasNotice" class="absolute inset-0 flex flex-col items-center justify-center p-6 text-center text-stone-400 pointer-events-none space-y-2">
                            ...
                            <p class="text-[10px] leading-relaxed">Klik menu di sebelah kiri untuk mulai menambahkan teks, foto, musik & kartu undangan.</p>
                        </div>
                ...
                <!-- Status Pill -->
                <div class="absolute bottom-6 right-6 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl border border-rosewarm-200 shadow-md flex items-center gap-2.5 text-xs text-warmgray font-semibold">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        // Cek jika ada penyimpanan lokal
